...
#validate the columns of the dataset
...
def validate_movies(df):
    ...
    return {"dataset": "movies.cleaned", "status": "PASSED" if not issues else "FAILED", "issues": issues}
...
def validate_ratings(df):
    ...
    return {"dataset": "ratings.cleaned", "status": "PASSED" if not issues else "FAILED", "issues": issues}
...
def validate_links(df):
    ...
    out = {"dataset": "links.cleaned", "status": status, "issues": issues}
...
def main():
    ...
    report = {
        "schema_check": {
            "movies.cleaned":  vr_m["status"],
            "ratings.cleaned": vr_r["status"],
            "links.cleaned":   vr_l["status"]
        },
        "details": {
            "movies.cleaned":  vr_m,
            "ratings.cleaned": vr_r,
            "links.cleaned":   vr_l
        }
    }
    ...
    print("Schema status:", report["schema_check"])
